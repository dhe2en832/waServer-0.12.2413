...
const maxRetryCountReceived =
  maxRetryReceived >= 1
    ? maxRetryReceived % 1 === 0
      ? maxRetryReceived
      : Math.ceil(maxRetryReceived)
    : 5;
...
const maxRetryCountSent =
  maxRetrySent >= 1 ? (maxRetrySent % 1 === 0 ? maxRetrySent : Math.ceil(maxRetrySent)) : 5;
...
const maxRetryCountStats =
  maxRetryStats >= 1 ? (maxRetryStats % 1 === 0 ? maxRetryStats : Math.ceil(maxRetryStats)) : 5;
        ...
        try {
          ...
          if (msg.hasQuotedMsg) {
            ...
            qmObj = {
              ...
              qm_base64:
                attachmentSave == true ? (qmhasMedia == false ? '-' : qmMedia.data) : 'disabled',
              qm_filename:
                attachmentSave == true
                  ? qmhasMedia == false
                    ? '-'
          ...
          const mainObj = {
            ...
            mimetype:
              attachmentSave == true ? (msg.hasMedia == false ? '-' : media.mimetype) : 'disabled',
            filename:
              attachmentSave == true ? (msg.hasMedia == false ? '-' : media.filename) : 'disabled',
            base64:
              attachmentSave == true ? (msg.hasMedia == false ? '-' : media.data) : 'disabled',
          };
          await new Promise((resolve, reject) => {
